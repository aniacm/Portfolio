...
function waterfall(parent,pin){
    ...
    $aPin.each( function( index, value ){
        ...
        if( index < num){
            ...
            console.log( pinH ,minH);
            if (minH<$( window ).scrollTop()+ $( window ).height()) {
                $(value).css({
                    'position': 'absolute',
                    'top': minH + 15,
...
function checkscrollside(){
    var $aPin = $( "#main>div" );
    var lastPinH = $aPin.last().get(0).offsetTop;//创建【触发添加块框函数waterfall()】的高度：最后一个块框的距离网页顶部+自身高的一半(实现未滚到底就开始加载)
